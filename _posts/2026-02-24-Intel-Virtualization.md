...
`VM entry` 명령은 VMCS의 guest-state area로부터 프로세서 상태를 로드합니다. VMM은 VM entry 시 인터럽트나 예외를 주입하도록 설정할 수 있는데, CPU가 게스트 IDT를 사용하여 마치 VM entry 직후 해당 이벤트가 발생한 것처럼 처리합니다. 이를 통해 VMM이 직접 인터럽트 전달 과정을 에뮬레이트할 필요가 없어집니다.

`VM exit` 시에는 프로세서 상태를 guest-state area에 저장하고 host-state area에서 불러옵니다. 모든 VM exit은 VMM의 공통 진입점을 사용하며, VM exit이 발생할 때마다 VMCS에 exit reason이 저장됩니다. 많은 경우 exit qualification도 함께 저장되어 더 자세한 정보를 제공합니다. 예를 들어 `MOV CR` 명령어가 VM exit을 트리거한 경우, exit reason은 "control-register access"를 나타내고 exit qualification에는 어떤 제어 레지스터인지, 읽기/쓰기 여부, 소스/목적지 범용 레지스터가 무엇인지가 기록됩니다.

VM entry와 VM exit 모두 CR3를 로드하기 때문에 VMM과 게스트는 서로 다른 선형 주소 공간에서 실행될 수 있습니다.

## Solving Virtualization Challenges with VT-X
VT-x는 게스트가 의도된 권한 레벨에서 작동할 수 있도록 하여 가상화의 문제점들을 해결합니다. 게스트는 권한 레벨이 아니라 VT-x로 인해 VMX non-root operation 모드로 실행된다는 점에서 제한을 받습니다. [그림](#virtualization-usage-models)의 2d에서 이를 설명하고 있습니다.

### Address-space compression
VT-x에서는 게스트와 VMM 간의 전환이 이루어질 때마다 CR3가 교체되어 선형 주소 공간이 바뀌기 때문에 게스트가 자신의 선형 주소 공간 전부를 사용할 수 있습니다. VMCS는 선형 주소 공간이 아닌 물리 주소 공간에 위치하기 때문에 게스트 선형 주소 공간을 침범하지 않습니다.

### Ring aliasing and ring compression
VT-x는 게스트가 의도된 권한 레벨에서 실행될 수 있게 하기 때문에 ring aliasing과 ring compression 문제가 해결됩니다. Ring aliasing의 경우 게스트 OS가 ring 0에서 직접 실행되므로 PUSH(CS) 같은 명령어가 실제 권한 레벨을 그대로 반환하여 가상화 여부가 노출되지 않습니다. Ring compression의 경우 게스트 OS가 ring 0, 게스트 앱이 ring 3에서 실행되므로 게스트 OS가 앱으로부터 자신을 정상적으로 보호할 수 있습니다.

### Nonfaulting access to privileged state
VT-x는 두 가지 방식으로 이 문제를 해결합니다. 첫 번째는 fault 없이 특권 상태에 접근하는 명령어가 VMM으로 전환되도록 지원을 추가하는 것이고, 두 번째는 해당 상태가 VMM에게 더 이상 중요하지 않도록 만드는 것입니다.

두 번째 방식이 핵심인데, VT-x에서는 VMCS가 인터럽트와 예외 처리를 담당하기 때문에 VMM이 GDTR, IDTR 같은 레지스터를 직접 관리할 필요가 없어집니다. 따라서 게스트가 ring 0에서 `LGDT, LIDT, LLDT, LTR, SGDT, SIDT, SLDT, STR` 같은 **명령어를 직접 실행** 할 수 있고, VMM이 굳이 가로채서 에뮬레이트할 필요가 없습니다.

### Guest transitions
게스트는 권한 레벨이 0이 아닐 때는 SYSENTER, SYSEXIT를 사용할 수 없었으나, VT-x 환경에서는 게스트 OS도 ring 0에서 실행될 수 있어 이러한 명령어들을 사용할 수 있습니다.

### Interrupt virtualization
VT-x는 VM-execution control에 `external-interrupt exiting` 제어 비트를 가지고 있습니다. 이 제어 비트가 1로 설정되면 게스트가 EFLAGS.IF를 수정할 때마다 VM exit을 발생시키지 않고도 **실제 인터럽트 제어권은 VMM이 유지** 할 수 있습니다. 실제 외부 인터럽트가 오면 게스트의 EFLAGS.IF 값과 무관하게 VM exit이 발생하고 VMM이 인터럽트를 받아서 처리합니다. 또한 `interrupt-window exiting` 제어 비트도 존재하는데, 이 비트가 1로 설정되면 게스트가 인터럽트를 받을 준비가 될 때마다(언마스킹) VM exit이 발생합니다. VMM이 게스트로 전달할 가상 인터럽트가 있을 때 이 비트를 설정하여 적절한 타이밍에 인터럽트를 주입할 수 있습니다.

### Access to hidden state
VT-x는 VMCS의 guest-state area에 소프트웨어로 접근할 수 없는 CPU 상태(descriptor cache 등)에 해당하는 필드를 포함합니다. 프로세서는 VM entry 시 이 필드들을 자동으로 로드하고 VM exit 시 자동으로 저장하여, VMM 실행 중이거나 VM을 전환할 때도 hidden state가 올바르게 보존됩니다.