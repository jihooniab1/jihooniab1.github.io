...
### 이중우선순위큐 (LV 3, 우선순위큐)
우선순위큐 사용 방법을 익히는 문제라고 보면 될 것 같다. 코딩테스트에서는 **heapq** 모듈로 우선순위큐를 구현한다. 기본적인 사용법은 다음과 같다. 주의해야할 점은, 이건 정렬이 아니라 **최소힙** 조건을 만족하게만 관리해주는 모듈이기 때문에, 맨 앞은 최솟값이 맞지만, **맨 뒤가 최댓값이 아니다**. 
...
이 점만 숙지하면 어렵지 않게 풀 수 있다. 

### 가장 먼 노드 (Lv 3, BFS)
그래프와 거리, 이런 키워드들을 보고 바로 다익스트라를 떠올릴 수 있지만, 
...
### 괄호 변환 (Lv 2, 재귀)
문제 설명대로 구현을 하면 어렵지 않게 풀 수 있었으나, 그렇게 하지 않아서 틀렸었다. 별개로 list랑 str은 달라서 str에서 reverse 메서드 같은걸 못 썼는데, 이에 대비하여 유용한 슬라이싱 패턴을 알아두면 좋을 것 같다.
```
a[::-1] # 뒤집기
a[i:j] # i~j-1 추출
a[::2] # 짝수 인덱스만 추출
...
### 거스름돈 (Lv 3, 재귀, DP)
재귀 함수를 짜고 메모이제이션을 통해 일반 테스트 케이스를 해결하는 거 까지는 어렵지 않게 할 수 있었는데, 자꾸 효율성 테스트 부분에서 시간 초과가 발생하였다.

원래 짠 코드는 큰 동전부터 0개일 때, 1개일 때,... 반복문을 돌려가면서 다음 크기의 동전을 계산하는 방식이었다. 정답을 맞추긴 했으나 시간 초과가 계속해서 발생하였다. 

1. 내 아이디어에서 한번 더 발전시키면 일일이 반복문을 돌리지 않고, 동전을 하나 더 넣을 건지, 안 넣고 다음 크기의 동전을 넣을 건지 경우의 합을 계산할 수 있다.
    - 현재 동전 안 쓰고 다음 동전으로: `rec(rest, pos+1)`
    - 현재 동전 1개 쓰고 같은 동전에서 다시 고민: `rec(rest - money[pos], pos)`
    이렇게 하면 반복문을 쓰지 않고 메모이제이션을 온전히 활용할 수 있게 된다. 

2. 여기서 한번 더 발전시키면 **Bottom-up** 방법을 떠올릴 수 있다. DP를 활용하는 것이다. dp[j]는 j원을 만들 수 있는 경우의 수를 나타내게 하는 것이다. 
    - `dp[j]` = j원을 만들 수 있는 경우의 수
    - `dp[0] = 1` (0원을 만드는 방법은 아무것도 안 쓰는 1가지)
    - 점화식: `dp[j] += dp[j - coin]`
     → "coin 하나를 추가했을 때, 나머지 j-coin원을 만드는 경우의 수를 그대로 가져온다"
