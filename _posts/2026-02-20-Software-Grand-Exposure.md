...
공격의 목표는 victim enclave가 입력 게놈 시퀀스를 전처리(인덱싱)하는 동안, STR 분석에 사용되는 13개 표준 위치에서의 microsatellite 반복 횟수(길이)를 캐시 모니터링을 통해 유출하는 것입니다. 공격 환경이 통제되어 victim의 실행이 결정론적이기 때문에, 캐시 모니터링 결과와 입력 시퀀스의 위치를 정확하게 대응시킬 수 있습니다.

캐시 부채널을 통해 victim의 캐시 접근 흔적은 해시 테이블 삽입 연산(Algorithm 1)과 연결될 수 있습니다. victim은 각 k-mer에 대해 **해시 함수로 idx를 계산하고, 해시 테이블에서 H[idx]가 가리키는 포인터를 조회** 합니다. 이 조회 과정에서 어느 캐시 라인이 활성화되는지 관찰하면 접근한 테이블 인덱스를 추론할 수 있고, 나아가 처리 중인 k-mer를 역추론할 수 있습니다.

그러나 여러 테이블 엔트리가 하나의 캐시 라인을 공유하기 때문에, 특정 캐시 라인이 축출되었을 때 정확한 인덱스 대신 k-mer 후보 집합만 알 수 있습니다. 이를 해결하기 위해 공격자는 타겟 STR 서열을 k-mer로 쪼개서 해당 STR이 처리될 때 활성화될 캐시 라인 패턴을 사전에 계산합니다. 예를 들어 ATCGC를 2-mer로 쪼개면 AT→TC→CG→GC 순서로 캐시 라인 0→3→1→2가 활성화됩니다. 실제 모니터링에서 이 패턴이 반복적으로 관찰되면 해당 STR이 처리되고 있음을 식별할 수 있습니다.

### Attack Results
![s5](/assets/img/posts/papers/softgrand_5.png) <br>

공격자는 게놈 시퀀스 문자열을 victim enclave에 입력으로 제공하면서 공격 코드를 병렬로 실행합니다. k=4로 설정하였으며, 이 경우 $4^4=256$가지 4-mer가 존재합니다. 각 해시 테이블 엔트리의 크기가 8바이트일 때 캐시 라인(64바이트)당 8개의 테이블 엔트리가 포함됩니다.

공격의 목표는 13개 표준 위치에서 microsatellite의 반복 횟수(길이)를 알아내는 것입니다. 예를 들어 CSF1PO 위치에서 반복되는 TAGA 서열의 길이를 알아내려 할 때, 각 4-mer가 매핑되는 캐시 라인을 사전에 계산합니다:
...
위 그림과 같이 4개의 캐시 라인을 개별적으로 모니터링하고 정렬하면, 입력 시퀀스에서 microsatellite가 나타날 때 4개의 캐시 라인이 모두 반복적으로 활성화됩니다. 이는 그림의 실선 네모와 같이 캐시 이벤트의 밀집도 증가로 확인할 수 있습니다.

4개의 캐시 라인이 동시에 활성화되어야 microsatellite로 판단하기 때문에 false positive 가능성은 매우 낮습니다(점선 네모). 캐시 모니터링 노이즈로 인해 정확한 길이를 알아내기는 어렵지만, 수동 분석 결과 $\pm 1$의 오차로 길이를 결정할 수 있음을 확인하였습니다.

각 표준 위치에서 STR 길이 후보가 3가지이므로, 유출된 정보와 일치하는 사람이 전체 인구에서 얼마나 되는지를 **CPI(Combined Probability of Inclusion)** 로 계산하였습니다. 최악의 경우, 즉 가장 흔한 유전자형을 가진 사람을 타겟으로 할 때도 CPI는 $7\times10^{-5}$로, 1000만 명 중 약 703명만이 유출된 정보와 일치합니다. 평균적인 경우에는 1000만 명 중 1명 미만이 일치하여 사실상 개인을 특정할 수 있습니다. 결론적으로 캐시 모니터링으로 유출된 불완전한 STR 정보만으로도 높은 확률로 개인을 식별할 수 있습니다.

## Countermeasure Analysis
캐시 부채널 공격을 막는 가장 확실한 방법은 캐시 자체를 비활성화하는 것이지만, enclave 실행 중에만 비활성화하더라도 대용량 데이터 처리나 암호화 연산을 수행하는 enclave 특성상 감당하기 어려운 수준의 성능 저하가 발생합니다.

캐시 하드웨어를 재설계하는 방법도 존재합니다. 두 가지 접근이 있는데, 하나는 캐시 메모리 내 접근을 무작위화하는 것이고, 다른 하나는 캐시 파티셔닝을 통해 보안에 민감한 코드가 신뢰할 수 없는 프로세스와 캐시를 공유하지 않도록 하는 것입니다. Sanctum 아키텍처는 LLC에 캐시 파티셔닝을 적용하고 enclave 종료 시 L1 캐시를 flush합니다. 그러나 이러한 하드웨어 변경은 제조사의 협력이 필수적이기 때문에 현실적으로 도입하기 어렵습니다.

**Oblivious RAM(ORAM)** 은 데이터를 지속적으로 섞고 재암호화하여 프로그램의 메모리 접근 패턴을 숨깁니다. 그러나 ORAM은 보통 server-client 모델에 적용되며 클라이언트가 상태 정보를 유지해야 합니다. 하드웨어 지원 없이 이 상태 정보를 캐시 부채널로부터 안전하게 보관하는 것이 불가능하기 때문에 캐시 보호에 직접 적용하기 어렵습니다. 공유 캐시를 주기적으로 비우거나 메모리 접근에 노이즈를 추가하는 난독화 기법도 제안되었으나, 성능 저하가 크고 victim과 병렬로 실행되는 공격자가 scrubbing/flush 사이에도 캐시 접근 패턴을 관찰할 수 있다는 한계가 있습니다.

애플리케이션 코드를 수정하는 방법도 있습니다. 첫 번째는 scatter-gather처럼 부채널로부터 이론적으로 안전한 구현으로 코드를 교체하는 것으로, 상당한 전문 지식과 노력을 필요로 합니다. 두 번째는 자동화된 컴파일러 변환 도구를 이용하는 것으로, 공격을 더 어렵게 만들거나 특정 유형의 부채널 공격을 방어할 수 있지만 완전한 보호는 어렵습니다.

SGX Shield 프레임워크는 enclave 코드를 무작위화하여 페이지 폴트 부채널 공격에 대한 방어를 제공합니다. 그러나 본 공격에는 직접적으로 적용되지 않는데, SGX Shield는 코드만 무작위화하고 데이터는 무작위화하지 않아 데이터 접근 패턴을 숨길 수 없기 때문입니다. 일반적으로 데이터 세그먼트 무작위화는 동적 메모리 할당, 해시 테이블과 같은 대형 데이터 구조, 그리고 포인터 연산 등으로 인해 구현이 매우 복잡합니다.

퍼포먼스 카운터를 이용한 시스템 레벨 캐시 공격 탐지 기법도 제안되었으나, SGX 공격자 모델에서는 공격자가 OS 권한으로 이러한 모니터링을 비활성화할 수 있어 무의미합니다. T-SGX와 Déjà Vu는 Intel TSX를 활용하여 잦은 enclave 인터럽트를 탐지하는 기법을 제안하였습니다. T-SGX는 인터럽트 탐지 시 실행을 종료하고 one-time token으로 재실행을 막지만, 공격자가 유효한 토큰을 먼저 탈취하면 enclave를 임의로 재실행할 수 있습니다. Déjà Vu는 counting thread를 타이머로 사용하여 인터럽트로 인한 실행 지연을 탐지하지만, 캐시 축출로 인한 지연 시 타이머 스레드도 함께 느려지고 공격자가 타이머 스레드를 낮은 주파수의 코어에 배치하여 탐지를 우회할 수 있습니다.